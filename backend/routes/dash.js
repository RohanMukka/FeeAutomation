...
router.get('/', cors.corsWithOptions, pass.verifyUser, function (req, res, next) {
  const query = "SELECT s.admno,s.name,s.programme,s.sem,s.dept,s.paidornot,c.scholname,f.deadline,DATE_FORMAT(f.deadline,'%d %M %Y') as dlday FROM students s,fees f,scholarships c WHERE s.sem=f.sem and s.programme=f.programme and s.scholid=c.scholid and s.admno=?";
  db.query(query,[req.user.username], function (err, result) {
    let fine = 'No';
...
router.get('/confirmation', cors.corsWithOptions, pass.verifyUser, function (req, res, next) {
  const query = "SELECT s.admno,s.name,s.programme,s.sem,s.dept,c.scholname,f.amount,f.deadline,DATE_FORMAT(f.deadline,'%d %M %Y') as dlday,c.concession FROM students s,fees f,scholarships c WHERE s.sem=f.sem and s.programme=f.programme and s.scholId=c.scholId and s.admno=?";
  db.query(query,[req.user.username], function (err, result) {
    let fine = 0;
    let today = new Date();
    let dayslate = Math.floor((today.getTime() - result[0].deadline.getTime()) / (1000 * 60 * 60 * 24));
